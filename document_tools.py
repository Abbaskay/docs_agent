...
def _is_empty(value) -> bool:
    if value is None:
        return True
    try:
        return bool(_WHITESPACE_ONLY_RE.fullmatch(str(value)))
    except Exception:
        return True
...
def _truncate_safe(text: str, max_len: int = MAX_OUTPUT_CHARS) -> str:
    if len(text) > max_len:
...
def generate_cv(
    ...
    """
    if _is_empty(full_name) or _is_empty(email) or _is_empty(phone) or _is_empty(location) or _is_empty(job_title):
        return "Please provide full_name, email, phone, location, and job_title to generate a CV."
...
def generate_cover_letter(
    ...
    """
    if _is_empty(full_name) or _is_empty(email) or _is_empty(phone) or _is_empty(job_title) or _is_empty(company_name):
        return "Please provide full_name, email, phone, job_title, and company_name to generate a cover letter."
    global _last_generated
    fields, truncation_note, error = _clean_many(
        {
            "full_name": full_name,
...
def generate_proposal(
    ...
    """
    if _is_empty(project_title) or _is_empty(prepared_by) or _is_empty(prepared_for):
        return "Please provide project_title, prepared_by, and prepared_for to generate a proposal."
    global _last_generated
    fields, truncation_note, error = _clean_many(
        {
...
def generate_report(
    ...
    """
    if _is_empty(report_title) or _is_empty(prepared_by):
        return "Please provide report_title and prepared_by to generate a report."
    global _last_generated
    fields, truncation_note, error = _clean_many(
        {
...
def generate_invoice(
    ...
    """
    if _is_empty(business_name) or _is_empty(business_email) or _is_empty(client_name) or _is_empty(client_email) or _is_empty(items):
        return "Please provide business_name, business_email, client_name, client_email, and items to generate an invoice."
    global _last_generated
    fields, truncation_note, error = _clean_many(
        {
    ...
    for line in items.strip().splitlines():
        ...
        if not line:
            continue
        line_count += 1
        if line_count > MAX_INVOICE_LINES:
            skipped_lines.append(f"[SKIPPED: too many lines, max {MAX_INVOICE_LINES}] {line}")
            continue
        ...
        try:
            ...
            price_str = re.sub(r"[^\d.\-]", "", parts[2])
            if not qty_str or not price_str:
                skipped_lines.append(f"[SKIPPED: missing qty or price] {line}")
                continue
            qty = Decimal(qty_str)
            unit_price = Decimal(price_str)
            if qty <= 0 or unit_price < 0:
                raise InvalidOperation
            if qty > Decimal("999999") or unit_price > Decimal("99999999"):
                skipped_lines.append(f"[SKIPPED: value out of range] {line}")
                continue
...
def generate_email(
    ...
    """
    if _is_empty(email_type) or _is_empty(sender_name):
        return "Please provide email_type and sender_name to generate an email."
    global _last_generated
    fields, truncation_note, error = _clean_many(
        {
...
def load_and_summarize(
    ...
    if _is_empty(content):
        return "Please provide document content to summarize."
    global _document
    fields, truncation_note, error = _clean_many(
        {
            "content": content,
...
def export_as_markdown(
    ...
    if not content:
        content = _last_generated.get("content", "")
    if _is_empty(content) and _is_empty(document_title):
